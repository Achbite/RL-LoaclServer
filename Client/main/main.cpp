...
static std::string BuildVizJson(const MazeEnv& env, const ClientConfig& cfg,
    ...
    // 墙壁（与 test_maze.json 一致）
    ss << "\"walls\":[";
    ss << "{\"x1\":0,\"y1\":0,\"x2\":20000,\"y2\":0,\"thickness\":100},";
    ss << "{\"x1\":20000,\"y1\":0,\"x2\":20000,\"y2\":20000,\"thickness\":100},";
    ss << "{\"x1\":20000,\"y1\":20000,\"x2\":0,\"y2\":20000,\"thickness\":100},";
    ss << "{\"x1\":0,\"y1\":20000,\"x2\":0,\"y2\":0,\"thickness\":100},";
    ...
    for (int ep = 0; ep < cfg.run.max_episodes; ++ep) {
        ...
        while (!env.AllDone()) {
            ...
            for (int i = 0; i < env.GetAgentNum(); ++i) {
                ...
                pos->set_x(info.pos_x);
                pos->set_y(info.pos_y);
            ...
            if (env.GetFrameId() % cfg.run.log_interval == 0) {
                ...
                float dx = a.pos_x - cfg.env.end_x;
                float dy = a.pos_y - cfg.env.end_y;
                float dist = std::sqrt(dx * dx + dy * dy);
                int act = last_actions.empty() ? 0 : last_actions[0];
                const char* act_name = (act >= 0 && act < 9) ? kActionNames[act] : "未知";
                LOG_INFO("Exec", "EP:%d F:%d | 指令:action=%d(%s) | 位置:(%.1f,%.1f) 距终点:%.1f",
                         ep, env.GetFrameId(), act, act_name, a.pos_x, a.pos_y, dist);
            }
        }
        ...
        // ---- 6. Episode 结束 ----
        {
            ...
            float dy = a.pos_y - cfg.env.end_y;
            float dist = std::sqrt(dx * dx + dy * dy);
            bool passed = (dist <= cfg.env.goal_radius);

            LOG_INFO("Result", "EP:%d 结束 | 帧数:%d | %s | 最终距离:%.1f",
                     ep, env.GetFrameId(), passed ? "通关" : "超时", dist);
